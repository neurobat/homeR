...
pmv <- function(clo, met, air.temp, saturation) {
  ...
  clo.area.fact <- ifelse(clothing.insulation < 0.078, 1 + 1.29 * clothing.insulation, 
  ...
  clo.surf.temp <- air.temp.kel + (35.5 - air.temp)/(3.5 * (6.45 * clothing.insulation + 
    0.1))
  ...
  therm.sens * (int.heat.prod - heat.loss.skin - heat.loss.sweat - heat.loss.respiration - 
    heat.loss.dry - heat.loss.rad - heat.loss.conv)
}
pmv <- Vectorize(pmv) 
