...
pmv <- function(clo, met, air.temp, saturation) {
  ...
  saturated.vapour.pressure <- exp(16.6536 - 4030.183/(air.temp + 
    235))  # kPa
  ...
  clo.area.fact <- ifelse(clothing.insulation < 0.078, 1 + 1.29 * 
    clothing.insulation, 1.05 + 0.645 * clothing.insulation)
  ...
  clo.surf.temp <- air.temp.kel + (35.5 - air.temp)/(3.5 * (6.45 * 
  ...
  repeat {
    ...
    if (all(abs(xn - xf) < eps)) 
      break
  }
  clo.surf.temp <- 100 * xn - 273
  # end iteration
  heat.loss.skin <- 3.05 * 0.001 * (5733 - 6.99 * int.heat.prod - 
    vapour.pressure)  # CIBSE Guide has an error here
  heat.loss.sweat <- pmax(0.42 * (int.heat.prod - 58.15), 0)
  heat.loss.respiration <- 1.7 * 1e-05 * met.rate * (5867 - vapour.pressure)
  ...
  therm.sens * (int.heat.prod - heat.loss.skin - heat.loss.sweat - 
    heat.loss.respiration - heat.loss.dry - heat.loss.rad - heat.loss.conv)
}
pmv <- Vectorize(pmv) 
